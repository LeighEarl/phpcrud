...
if (isset($_GET['id_no'])) {
    if (!empty($_POST)) {
        ...
        // Update the record
        $stmt = $pdo->prepare('UPDATE contacts SET id_no = ?, first_name = ?, middle_name = ?, last_name = ?, email = ?, phone = ?, address = ?, sex = ?, course = ?, password = ?, created = ? WHERE id_no = ?');
    ...
    $stmt = $pdo->prepare('SELECT * FROM contacts WHERE id_no = ?');
